...
var BarcodeReader = {
  ...
  Init: function() {
	...
	BarcodeReader.ScanCanvas = document.createElement("canvas");
  ...
  BarcodeReaderImageCallback: function(e) {
	...
	if (e.data.success === "image") {
      BarcodeReader.ImageCallback(e.data);
      return;
    }
  ...
  stepped_scale: function(canvas, img, width, step) {
  ...
  BarcodeReaderDecodeImage: function(image, orientation, sceneCaptureType) {
	console.log(orientation, sceneCaptureType);
	const ratio = image.height > image.width ? image.height / image.width : image.width / image.height;
	/*const maxHeight = 1280;
	const maxWidth = 1280 / ratio;
	...
	BarcodeReader.ScanCanvas.width = maxWidth;
    BarcodeReader.ScanCanvas.height = maxHeight;*/
	
	const desiredHeight = Math.min(3000, image.height)//(document.body.clientHeight - 50) * 1.5; //1280;
	const desiredWidth = Math.round(desiredHeight / ratio); //720;
	BarcodeReader.ScanCanvas.width = desiredWidth;
    BarcodeReader.ScanCanvas.height = desiredHeight;
	...
	while (cur.width * 0.5 > desiredWidth) {
     cur = {
	...
	BarcodeReader.ScanContext.drawImage(oc, 0, 0, cur.width, cur.height, 0, 0, desiredWidth, desiredHeight);*/
	BarcodeReader.stepped_scale(BarcodeReader.ScanCanvas, image, desiredWidth, 0.5);
	var c = document.getElementById("barcode-reader__picture");
	c.width = desiredWidth/2;
	c.height = desiredHeight/2;
	c.getContext('2d').drawImage(BarcodeReader.ScanCanvas, 0, 0, desiredWidth/2, desiredHeight/2);
	//return;
