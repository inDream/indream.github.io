(function ( $ ) {
  $.fn.barcodeReader = function(options) {
    ...
    var $inputEl = ($userInputEl.length > 0) ? $userInputEl : $('<input id="barcode-reader__picture__input" type="file" accept="image/*" capture />');
    ...
    if(settings.video){
      ...
      BarcodeReader.SetImageCallback(function(result) {
		...
		if (result.success === "image") {
			...
			imgData.data.set(result.result);
			cctx.putImageData(imgData, 0, 0);
			var loc = localized[result.i];
			ctx.drawImage(c, loc.x/2, loc.y/2 + 50, loc.width/2, loc.height/2);
			return;
		}
      ...
      BarcodeReader.SetLocalizationCallback(function(result) {
        ...
        for (var i = 0; i < result.length; i++) {
          ctx.rect(result[i].x/2, result[i].y/2, result[i].width/2, result[i].height/2);
        }
        ctx.stroke();
      });
      ...
      // create the dom elements needed
      $mainEl.append([ $inputEl, $canvasEl, $resultEl ]);
    }
